...
namespace NCL {
	...
	namespace CSC8503 {
		...
		struct QuadTreeEntry {
			...
			QuadTreeEntry(GameObject* obj, Vector3 pos, Vector3 size) {
				object		= obj;
				this->pos	= pos;
				this->size	= size;
			}
		...
		class QuadTreeNode	{
		public:
			typedef std::function<void(std::list<QuadTreeEntry>&)> QuadTreeFunc;
		protected:
			...
			void Insert(GameObject* object, const Vector3& objectPos, const Vector3& objectSize, int depthLeft, int maxSize) {
                ...
                if (children) {
                    ...
                    contents.push_back(QuadTreeEntry(object, objectPos, objectSize));
	...
	namespace CSC8503 {
		...
		public:
			QuadTree(Vector2 size, int maxDepth = 6, int maxSize = 5){
				root = QuadTreeNode(Vector2(), size);
